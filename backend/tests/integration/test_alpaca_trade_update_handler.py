...
def _seed_submitted_order(quantity: str = "10") -> tuple[uuid.UUID, str]:
    with _owner_uow() as uow:
        ...
        uow.commit()
    return order_id, order.client_order_id


def _handler() -> AlpacaTradeUpdateHandler:
    return AlpacaTradeUpdateHandler(uow_factory=_owner_uow, now=lambda: NOW)


def test_new_event_maps_to_accepted() -> None:
    order_id, client_order_id = _seed_submitted_order()
...
def test_fill_event_updates_filled_quantity_and_average_price() -> None:
    order_id, client_order_id = _seed_submitted_order(quantity="10")
