"""Creates a billing period's pending charge, applies the Stripe outcome, posts the ledger entry (S10 §5, ADR 10).

Locks to the as-published watermark, never a live figure (FR-47).
"""
...
class FeeChargeService:
    ...
    def create_pending_charge(
        ...
        """`None` when `total_accrued` is zero, or this period's charge already exists.

        Read-then-insert is a fast path; `uq_fee_charge_customer_period` + a `SAVEPOINT` are the
        real idempotency backstop against a concurrent duplicate (F5 fix, S0 §10.1 audit).
        """
