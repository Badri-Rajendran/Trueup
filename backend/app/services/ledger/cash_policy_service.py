"""Pure cash policy functions everything else reads instead of a stored balance (S1 §5, ADR 5)."""
...
class CashPolicyService:
    ...
    def settled_cash(self, customer_id: uuid.UUID) -> Money:
        """Cash postings needing no settlement, already confirmed, or a pending outflow (§5)."""
    ...
    def unsettled_deposit_proceeds(self, customer_id: uuid.UUID) -> Money:
        """A deposit is investable immediately, before its own obligation confirms (S1 §5)."""
    ...
    def _as_money(value: Money | None) -> Money:
        """Narrows the nullable column type; SQL's `COALESCE` guarantees a non-null value."""
    ...
    def investable(self, customer_id: uuid.UUID) -> Money:
        """Counts unsettled inflows too; deliberately not given to `withdrawable` (ADR 5)."""
        return (
