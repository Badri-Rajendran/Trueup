...
`assert projection == fold(order_event)` holds from `submitted` onward. The pre-submission
lifecycle (`draft -> awaiting_approval -> approved`) has no `event_type`; `OrderService` writes it
directly (S1's `account`-vs-ledger precedent).
"""
...
class OrderStatus(StrEnum):
    """S3 §4's state machine. `draft` is transient; no committed row is ever left at `draft`."""
...
"""FR-38's terminal non-filled state set; triggers `ApprovalHoldService.release`."""


def derive_client_order_id(order_id: uuid.UUID) -> str:
    """Deterministic, derived from `order.id`, never regenerated on retry (S3 §3.1/§5)."""
...
class Order(Base):
    ...
    # No FK: the securities catalogue is S5's to define (matches account.security_id, S1 §3.1).
...
class OrderRepository(BaseRepository[Order]):
    ...
    def open_buy_orders(self, customer_id: uuid.UUID) -> list[Order]:
        """Buy orders past the approval-hold window but not yet resolved (S1 §5 `open_buy_commitments`)."""
