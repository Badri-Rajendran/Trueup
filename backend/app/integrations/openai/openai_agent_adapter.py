...
Sync-to-async bridge: `_run_in_thread` runs `Runner.run_streamed()` on a dedicated background
thread's event loop, relaying events to the sync generator via a `queue.Queue` (Flask views are
synchronous, S0 §3). `_get_database_schema_tool`/`_execute_read_only_sql_tool` are thin wrappers
around whatever `ChatToolContext` the orchestration service supplied.
"""
...
async def _execute_read_only_sql_tool(ctx: RunContextWrapper[ChatToolContext], sql: str) -> str:
    """Executes one read-only SELECT against the curated views; a failure returns `{"error": ...}`,
    never a raw database error string."""
    start = time.monotonic()
    ...
    async def _produce(
        ...
        try:
            ...
            result = Runner.run_streamed(
                agent,
                input_items,  # type: ignore[arg-type] -- SDK stubs can't match list[dict[str, Any]]
            ...
            # Never a raw provider error to the customer (S11 §5.3, OWASP A05).
