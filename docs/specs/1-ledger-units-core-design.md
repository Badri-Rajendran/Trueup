...
1. **Money sums to zero per entry** — property-based test posting arbitrary valid entries;
   assert the invariant on every `entry_type` example in §3.4.
...
constraints in §3.3 and the revoked `UPDATE`/`DELETE` grants in §6 must be part of the migration,
not left to application-layer discipline alone.
