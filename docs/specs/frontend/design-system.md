...
The concrete reference point is a **ledger, not a dashboard** — the object this whole system is
built around (`ADR 1`: an append-only, bitemporal posting ledger; `docs/specs/6-restatement-engine.md`;
`docs/specs/5-tax-lots-and-corporate-actions.md`). A well-kept ledger is quiet, exact, and legible at
a glance: flat pages, ruled lines, ink on paper, figures that line up in columns. That vocabulary —
hairline rules instead of drop shadows, restrained near-black "ink" instead of a bright brand color,
tabular figures that align — is what this system is built from. It is a deliberate departure from
both the warm-cream/serif-display and the near-black/neon-accent looks that AI-generated design
defaults to; neither reads as "audited," which is the actual job here.
...
| Token | Hex | Role |
| --- | --- | --- |
| `ink` | `#12202E` | Primary text, headings, primary button fill — the "ledger ink" |
| `accent` (verdigris) | `#1F6E68` | Interactive elements: links, focus, selected state, the as-corrected/restated indicator, primary chart line |
| `success` | `#1E6B3C` | Settled, reconciled, approved, gains |
| `warning` | `#9C5B12` | Pending, aging, needs-attention-soon |
| `error` | `#A32E27` | Reconciliation break, failed/bounced, rejected, losses |
| `paper` | `#F5F6F7` | App background — cool, near-neutral, not warm cream |
...
brand blue — it reads as "fountain-pen ink on a ledger page," not "corporate blue." `accent` is a
deep verdigris (aged-copper patina, the color bank fixtures and old ledger bindings actually oxidize
to) rather than the generic SaaS blue (`#4F7CFF`-family) or the AI-cliché terracotta — it is used
narrowly enough (§2.4) that its scarcity is part of the signal. `success`/`error` are deep forest
green and brick red rather than saturated traffic-light green/red — a callback to the literal
double-entry convention of red ink for a negative figure, deep enough to read as considered rather
than a dashboard warning light.
...
| --- | --- | --- | --- |
| `--color-paper` | `#F5F6F7` | `#0F151B` | App canvas background |
| `--color-surface` | `#FFFFFF` | `#1A222B` | Card/panel background |
| `--color-surface-sunken` | `#ECEEF0` | `#141B22` | Table row stripe, inset wells |
| `--color-hairline` | `#DBDFE3` | `#2B343E` | Default border/divider |
| `--color-border-strong` | `#B9C0C7` | `#3E4954` | Input borders, emphasized dividers |
| `--color-text` | `#12202E` | `#E9EDF0` | Primary text |
| `--color-text-secondary` | `#3A4149` | `#B7C0C9` | Secondary text, labels |
| `--color-text-muted` | `#5B6570` | `#7C8792` | Captions, timestamps, disabled — never for primary data |
| `--color-accent` | `#1F6E68` | `#4FBFB0` | Links, focus ring, selected state, restated indicator |
| `--color-accent-strong` | `#175450` | `#3AA394` | Hover/pressed accent |
| `--color-accent-subtle` | `#E3EFEC` | `#12332E` | Selected-row tint, accent-tinted chip background |
| `--color-success` | `#1E6B3C` | `#4CAF6E` | Text/icon on success state |
| `--color-success-subtle` | `#E5F2E9` | `#14301F` | Success badge/banner background |
| `--color-warning` | `#9C5B12` | `#D9A441` | Text/icon on warning state |
| `--color-warning-subtle` | `#FBF0DF` | `#3A2C10` | Warning badge/banner background |
| `--color-error` | `#A32E27` | `#E2635A` | Text/icon on error state |
| `--color-error-subtle` | `#FBEAE8` | `#3A1613` | Error badge/banner background |
...
| Pairing | Ratio | Floor |
| --- | --- | --- |
| `text` on `paper` (light) | 15.3:1 | AAA (7:1) |
| `text-secondary` on `paper` | 9.6:1 | AAA |
| `text-muted` on `paper` | 5.5:1 | AA (4.5:1) — captions/timestamps only, see below |
| `accent` on `paper`/`surface` | 5.6–6.0:1 | AA |
| `success` on `paper` | 6.0:1 | AA |
| `error` on `paper` | 6.5:1 | AA |
| `warning` on `paper` | 5.0:1 | AA |
| white text on `ink` (primary button) | 15.3:1 | AAA |
| white text on `accent` (accent button) | 6.0:1 | AA |
| `text` on `paper` (dark) | 15.6:1 | AAA |
| `accent`/`success`/`warning` on `paper` (dark) | 6.7–8.2:1 | AA/AAA |
| `error` on `paper` (dark) | 5.4:1 | AA |
...
- **`accent` (verdigris) is reserved** for: interactive elements (links, focus rings), the current
